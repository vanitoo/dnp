...
const DNP_VERSION = '3.14.2';
const DNP_ADMIN_PASSWORD = '123456';
const DNP_PDF_LOG_ENABLED = true;
...
function dnpIsTransientGoogleApiError_(error) {
  const text = String(error && error.message ? error.message : error || '').toLowerCase();
  return /internal error|backenderror|backend error|service unavailable|temporar|rate limit|ratelimit|quota|too many requests|http 429|http 500|http 502|http 503|http 504/.test(text);
}

function dnpWithRetry_(operationName, callback, maxAttempts) {
  ...
  for (let attempt = 1; attempt <= attempts; attempt++) {
    try {
      ...
      lastError = error;
      if (attempt >= attempts || !dnpIsTransientGoogleApiError_(error)) {
        throw error;
      }

      const delayMs = 500 * Math.pow(2, attempt - 1); // 500, 1000, 2000 ms
      console.warn(
        operationName + ': временная ошибка Google API, попытка ' +
        attempt + '/' + attempts + '. Повтор через ' + delayMs + ' мс. ' +
...
function dnpDriveCopy_(source, name, parent) {
  ...
  return dnpWithRetry_('Drive.Files.copy', function() {
...
function onOpen() {
  ...
  const templateMenu = ui.createMenu('Шаблон квитанции')
    .addItem('Открыть шаблон', 'openReceiptTemplate')
    .addItem('Создать новый шаблон…', 'createReceiptTemplateForCurrentFormat');

  ui.createMenu('ДНП')
    .addSubMenu(ui.createMenu('Квитанции')
      .addItem('Сформировать PDF…', 'showPdfDialog')
      .addItem('Открыть папку квитанций', 'openReceiptsFolder'))
    .addSubMenu(ui.createMenu('Почта')
      .addItem('Отправить квитанции…', 'sendReceipts')
      .addSeparator()
      .addItem('Получатели', 'openEmailsSheet')
      .addItem('Журнал отправки', 'openJournalSheet'))
    .addSubMenu(ui.createMenu('Данные и настройка')
      .addItem('Настройки', 'openSettingsSheet')
      .addItem('Тарифы', 'openTariffsSheet')
      .addSeparator()
      .addItem('Создать новый год…', 'showCreateYearDialog')
      .addItem('Подготовить / обновить лист года', 'prepareCurrentYearSheet')
      .addItem('Добавить услугу…', 'showAddServiceRowDialog')
      .addSeparator()
      .addSubMenu(templateMenu))
    .addSubMenu(ui.createMenu('Сервис')
      .addItem('Первичная настройка…', 'showInitialSetupDialog')
      .addItem('Удалить PDF за месяц…', 'showDeletePdfsMonthDialog')
      .addSeparator()
      .addItem('Очистить журнал отправки', 'clearJournal'))
    .addSeparator()
...
function escapeDialogHtml_(v){return String(v==null?'':v).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\\r?\\n/g,'<br>')}
function submitForm(){const b=document.getElementById('submit'),s=document.getElementById('status'),p=document.getElementById('password');b.disabled=true;s.textContent='Формирование запущено…';google.script.run.withSuccessHandler(r=>{const msg=(r&&r.message)?String(r.message):'';document.body.innerHTML='<h2>Формирование завершено</h2><div class="info">Создано PDF: <b>'+Number(r.created||0)+'</b><br>Ошибок: <b>'+Number(r.failed||0)+'</b>'+(msg?'<br><br>'+escapeDialogHtml_(msg):'')+'</div><div class="buttons"><button class="primary" onclick="google.script.host.close()">Закрыть</button></div>'}).withFailureHandler(e=>{s.textContent='Ошибка: '+e.message;b.disabled=false}).startPdfGenerationFromDialog(Number(document.getElementById('year').value),Number(document.getElementById('month').value),p?p.value:'')}
refreshCount();
...
function openReceiptsFolder() {
  const folder = getDnpPdfFolder_();
  showDriveLinkDialog_('Квитанции ДНП Комфорт', folder.name || 'Квитанции', dnpDriveFolderUrl_(folder));
}
...
function openTariffsSheet() { showAndActivateSheet_(DNP_TARIFF_SHEET_NAME); }
...
function prepareCurrentYearSheet() {
  const sheet = SpreadsheetApp.getActiveSheet();
  if (!/^\d{4}$/.test(sheet.getName())) {
    SpreadsheetApp.getUi().alert('Откройте лист нужного года, например 2026.');
    return;
  }

  fillKwhFormulas();
  fillSumFormulasFromTariffs();
  formatYearSheetUX();

  SpreadsheetApp.getActive().toast(
    'Формулы кВтч и суммы обновлены, оформление применено.',
    'Лист ' + sheet.getName() + ' подготовлен',
    8
  );
}
...
function generatePdfsForMonth(year, month, password) {
  ...
  blocks.forEach((block, index) => {
    ...
    try {
      ...
      failed++;
      const errorText = 'Участок ' + block.plot + ': ' + (error && error.message ? error.message : error);
      errors.push(errorText);
      console.error('PDF generation failed: ' + errorText);
      if (DNP_PDF_LOG_ENABLED) {
        try { appendPdfLog_('', 'УЧАСТОК', year, month, block.plot, 'ERROR', errorText); } catch (logError) {}
      }
      if (tempFileId) {
...
function showDeletePdfsMonthDialog() {
  const years = getYearSheetNames_();
  if (!years.length) {
    SpreadsheetApp.getUi().alert('Не найдено ни одного листа с названием года.');
    return;
  ...
  const monthOptions = getRussianMonthNameList_().map((name, index) => {
    const month = index + 1;
    return '<option value="' + month + '"' + (month === currentMonth ? ' selected' : '') + '>' +
      String(month).padStart(2, '0') + ' ' + name + '</option>';
  }).join('');
...
.note{background:#fff4e5;padding:10px;border-radius:6px;margin-top:14px;line-height:1.45}.buttons{display:flex;justify-content:flex-end;gap:8px;margin-top:18px}
button{padding:9px 14px;border:0;border-radius:6px;cursor:pointer}.danger{background:#d93025;color:white}.secondary{background:#f1f3f4}#status{margin-top:12px;min-height:20px;color:#5f6368}
</style></head><body>
<h2>Удалить PDF за месяц</h2>
<label>Год</label><select id="year">${yearOptions}</select>
<label>Месяц</label><select id="month">${monthOptions}</select>
<div class="note">Будут перемещены в корзину только PDF из выбранной папки месяца. Другие месяцы и годы не затрагиваются.</div>
<div id="status"></div>
<div class="buttons"><button class="secondary" onclick="google.script.host.close()">Отмена</button><button id="deleteBtn" class="danger" onclick="runDelete()">Удалить PDF</button></div>
<script>
function runDelete(){
  const year=Number(document.getElementById('year').value),month=Number(document.getElementById('month').value);
  const monthText=document.getElementById('month').selectedOptions[0].textContent;
  if(!confirm('Удалить PDF за '+monthText+' '+year+'?\\n\\nФайлы будут перемещены в корзину.'))return;
  const b=document.getElementById('deleteBtn'),s=document.getElementById('status');b.disabled=true;s.textContent='Удаление…';
  google.script.run.withSuccessHandler(r=>{s.textContent=r.message;b.disabled=false;}).withFailureHandler(e=>{s.textContent='Ошибка: '+e.message;b.disabled=false;}).deletePdfsForMonth(year,month);
}
</script></body></html>`).setWidth(460).setHeight(390);

  SpreadsheetApp.getUi().showModalDialog(html, 'ДНП');
}

function deletePdfsForMonth(year, month) {
  year = Number(year);
  month = Number(month);
  if (!Number.isInteger(year) || year < 2000 || year > 2100) throw new Error('Некорректный год.');
  if (!Number.isInteger(month) || month < 1 || month > 12) throw new Error('Некорректный месяц.');
  ...
  if (!yearFolder) throw new Error('Папка года «' + year + '» не найдена.');

  const monthName = String(month).padStart(2, '0') + ' ' + getRussianMonthName_(month);
  const monthFolder = findChildFolderByNames_(yearFolder, [monthName]);
  if (!monthFolder) throw new Error('Папка «' + monthName + '» не найдена.');

  const result = trashDnpPdfFilesRecursively_(monthFolder);
  const message = 'Перемещено в корзину PDF: ' + result.deleted + (result.failed ? '. Ошибок: ' + result.failed : '') + '.';
  SpreadsheetApp.getActive().toast(message, year + ' / ' + monthName, 8);
  return { ok: result.failed === 0, deleted: result.deleted, failed: result.failed, message };
}

function getRussianMonthNameList_() {
...
function getDnpPdfFolder_() {
  ...
  if (!id) {
    throw new Error(
      ...
      'Укажите ID папки «Квитанции ДНП Комфорт» или выполните «ДНП → Сервис → Первичная настройка…».'
    );
  }
